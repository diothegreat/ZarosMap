...
### Ροή πληρωμής: «κράτηση τώρα, χρέωση 10 ημέρες πριν την άφιξη»

Ο πελάτης **δεν χρεώνεται** τη στιγμή της κράτησης — η κάρτα του απλώς αποθηκεύεται με ασφάλεια (Stripe Checkout σε `setup` mode). Η πραγματική χρέωση γίνεται **αυτόματα 10 ημέρες πριν την άφιξη**, μόλις λήξει το δωρεάν παράθυρο ακύρωσης. Έτσι, όποιος ακυρώσει νωρίς **δεν χρεώνεται ποτέ** και δεν χάνετε προμήθεια σε ακυρώσεις.

Endpoints:

- `GET /api/availability?unit=maera|dionysos` — κατειλημμένες ημερομηνίες (αποτρέπει διπλοκράτηση· οι εκκρεμείς κρατήσεις δεσμεύουν ημερομηνίες για 30′)
- `POST /api/reserve` — δημιουργεί Stripe Checkout `setup` session (αποθήκευση κάρτας, **καμία χρέωση**)· η τιμή υπολογίζεται **πάντα στον server**
- `POST /api/webhook` — στο `checkout.session.completed` αποθηκεύει την κάρτα, κλειδώνει τις ημερομηνίες και στέλνει email επιβεβαίωσης
- `POST /api/cancel` — δωρεάν ακύρωση έως 10 ημέρες πριν (αν είχε ήδη γίνει χρέωση, γίνεται refund)
- `POST /api/cron/charge-due` — χειροκίνητη εκτέλεση του job χρέωσης (τρέχει ούτως ή άλλως αυτόματα κάθε 6 ώρες)

Η χρέωση 10 ημερών πριν την άφιξη εκτελείται αυτόματα από τον ίδιο τον server (timer κάθε 6 ώρες + στην εκκίνηση). Οι κρατήσεις αποθηκεύονται στο `server/data/bookings.json`.
